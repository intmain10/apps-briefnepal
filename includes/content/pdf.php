...
'merge-pdf' => [
    ...
    'intro' => [
        ...
        'This merger keeps the underlying page objects intact rather than flattening them to pictures, so text stays selectable and searchable in the merged file, links and fonts survive, and page dimensions are preserved even when you mix A4 and US Letter originals in the same document.',
        'You are not limited to appending whole files end to end. Each file has a page selector, so you can take only pages 1-3 of one document and page 7 of another, and there is an insert mode that treats your first file as the base and drops the others in before a page number you choose. That is the difference between rebuilding a contract by hand and simply dropping a re-signed page 12 back into the original where it belongs.',
    ],
    'faqs' => [
        ['Can I merge only certain pages instead of whole files?', 'Yes. Every file in the list has a "Pages to take" box that accepts numbers and ranges such as 1-3, 7. Leave it blank to include the whole file. Pages outside the document are ignored rather than causing an error.'],
        ['Can I insert one PDF into the middle of another?', 'Yes. Switch the mode to insert, and your first file becomes the base document. Every other file then gets an "Insert before page" box, so a two-page appendix can land before page 5 rather than at the end. Leave the box blank to append at the end, and note the page number refers to the base file\'s original numbering, so positions do not shift as other insertions are applied.'],
